...
public final class DocumentationPrinter {
    ...
    private static void writePermissionsWikiTable(PrintStream stream, List<Class<?>> commandClasses, String prefix) {
        ...
        for (Class<?> cls : commandClasses) {
            for (Method method : cls.getMethods()) {
                ...
                if (method.isAnnotationPresent(NestedCommand.class)) {
                    ...
                    writePermissionsWikiTable(stream, Arrays.asList(nestedClasses), prefix + cmd.aliases()[0] + " ");
                }
            }
        }
    }
