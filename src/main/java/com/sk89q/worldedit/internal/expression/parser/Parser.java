...
public class Parser {
                            ...
                            if (peek().id() == ';') {
                                ...
                                if (!(equalsToken instanceof OperatorToken)
                                    ...
                                    throw new ParserException(
                                        variableToken.getPosition(),
                                        "Expected '=' or a term and ';'");
                                }
                                ++position;
                                ...
                                statements
                                    .add(new SimpleFor(current.getPosition(), (LValue) variable, first, last, body));
                            } // switch (keyword.charAt(0))
                            break;
                        }
                            ...
                            while (peek().id() != '}') {
                                ...
                                if (hasKeyword("case")) {
                                    ...
                                    if (!(valueToken instanceof NumberToken)) {
                                        throw new ParserException(
                                            current.getPosition(),
                                            "Expected number instead of " + peek());
                                    }
                                    ...
                                    throw new ParserException(
                                        current.getPosition(),
                                        "Expected 'case' or 'default' instead of " + peek());
                                }
                            }
                            consumeCharacter('}');

                            statements
                                .add(new Switch(current.getPosition(), parameter, values, caseStatements, defaultCase));
                            break;
            ...
            switch (current.id()) {
                ...
                case 'i':
                    ...
                    if (next.id() == '(') {
                        ...
                        if (variable == null) {
                            halfProcessed
                                .add(new UnboundVariable(identifierToken.getPosition(), identifierToken.value));
                        } else {
    ...
    private Function parseFunctionCall(IdentifierToken identifierToken) throws ParserException {
        ...
        try {
            ...
            throw new ParserException(
                identifierToken.getPosition(),
                "Function '" + identifierToken.value + "' not found",
                e);
        }
    }
