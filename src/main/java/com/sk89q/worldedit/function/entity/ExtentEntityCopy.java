...
public class ExtentEntityCopy implements EntityFunction {
    ...
    public boolean apply(Entity entity) throws WorldEditException {
        ...
        if (state != null) {
            ...
            Vector pivot = from.round()
                .add(0.5, 0.5, 0.5);
            Vector newPosition = transform.apply(
                location.toVector()
                    .subtract(pivot));
            Vector newDirection;

            newDirection = transform.isIdentity() ? entity.getLocation()
                .getDirection()
                : transform.apply(location.getDirection())
    ...
    private BaseEntity transformNbtData(BaseEntity state) {
        ...
        if (tag != null) {
            ...
            if (hasTilePosition) {
                ...
                Vector newTilePosition = transform.apply(tilePosition.subtract(from))
                    .add(to);
                ...
                if (hasDirection || hasLegacyDirection) {
                    ...
                    if (direction != null) {
                        Vector vector = transform.apply(direction.toVector())
                            .subtract(transform.apply(Vector.ZERO))
                            .normalize();
