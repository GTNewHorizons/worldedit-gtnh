...
public class PlatformManager {
    ...
    public synchronized boolean unregister(Platform platform) {
        ...
        if (removed) {
            logger.log(
                Level.FINE,
                "Unregistering " + platform.getClass()
                    .getCanonicalName() + " from WorldEdit");
            ...
            Iterator<Entry<Capability, Platform>> it = preferences.entrySet()
                .iterator();
            while (it.hasNext()) {
    ...
    private synchronized void choosePreferred() {
        ...
        if (preferences.containsKey(Capability.CONFIGURATION) && configured.compareAndSet(false, true)) {
            worldEdit.getEventBus()
                .post(new ConfigurationLoadEvent(queryCapability(Capability.CONFIGURATION).getConfiguration()));
        }
    }
    ...
    private synchronized @Nullable Platform findMostPreferred(Capability capability) {
        ...
        for (Platform platform : platforms) {
            Preference preference = platform.getCapabilities()
                .get(capability);
    ...
    public void handlePlatformReady(PlatformReadyEvent event) {
        ...
        if (initialized.compareAndSet(false, true)) {
            worldEdit.getEventBus()
                .post(new PlatformInitializeEvent());
        }
    }
            ...
            if (event.getType() == Interaction.HIT) {
                ...
                if (tool != null && tool instanceof DoubleActionBlockTool) {
                    if (tool.canUse(player)) {
                        ((DoubleActionBlockTool) tool).actSecondary(
                            queryCapability(Capability.WORLD_EDITING),
                            getConfiguration(),
                            player,
                            session,
        ...
        switch (event.getInputType()) {
            ...
            case SECONDARY: {
                ...
                if (tool != null && tool instanceof TraceTool) {
                    if (tool.canUse(player)) {
                        ((TraceTool) tool)
                            .actPrimary(queryCapability(Capability.WORLD_EDITING), getConfiguration(), player, session);
                        event.setCancelled(true);
                        return;
                    }
                }
