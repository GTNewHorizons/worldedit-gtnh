 ...
 */
public class CylinderRegionSelector extends com.sk89q.worldedit.regions.CylinderRegionSelector
    implements RegionSelector, CUIRegion {

    protected static transient final NumberFormat NUMBER_FORMAT;
    protected transient CylinderRegion region;

    static {
        NUMBER_FORMAT = (NumberFormat) NumberFormat.getInstance()
            .clone();
    ...
    public CylinderRegionSelector(RegionSelector oldSelector) {
        this(
            checkNotNull(oldSelector).getIncompleteRegion()
                .getWorld());
        ...
        if (oldSelector instanceof CylinderRegionSelector) {
            ...
            Vector center = pos1.add(pos2)
                .divide(2)
                .floor();
            region.setCenter(center.toVector2D());
            region.setRadius(
                pos2.toVector2D()
                    .subtract(center.toVector2D()));
    ...
    public boolean selectPrimary(Vector position, SelectorLimits limits) {
        if (!region.getCenter()
            .equals(Vector.ZERO) && position.compareTo(region.getCenter()) == 0) {
            return false;
        }
    ...
    public boolean selectSecondary(Vector position, SelectorLimits limits) {
        ...
        final Vector2D diff = position.subtract(center)
            .toVector2D();
    ...
    public void explainSecondarySelection(Actor player, LocalSession session, Vector pos) {
        ...
        if (!center.equals(Vector.ZERO)) {
            player.print(
                "Radius set to " + NUMBER_FORMAT.format(
                    region.getRadius()
                        .getX())
                    + "/"
