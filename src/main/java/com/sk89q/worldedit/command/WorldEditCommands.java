...
public class WorldEditCommands {
    ...
    public void version(Actor actor) throws WorldEditException {
        ...
        for (Capability capability : Capability.values()) {
            ...
            actor.printDebug(
                String.format("%s: %s", capability.name(), platform != null ? platform.getPlatformName() : "NONE"));
        }
    }
    ...
    public void reload(Actor actor) throws WorldEditException {
        ...
        we.getEventBus()
            .post(
                new ConfigurationLoadEvent(
                    we.getPlatformManager()
    ...
    public void cui(Player player, LocalSession session, EditSession editSession, CommandContext args)
    ...
    public void tz(Player player, LocalSession session, EditSession editSession, CommandContext args)
        throws WorldEditException {
        TimeZone tz = TimeZone.getTimeZone(args.getString(0));
        session.setTimezone(tz);
        player.print("Timezone set for this session to: " + tz.getDisplayName());
        player.print(
            "The current time in that timezone is: " + dateFormat.format(
