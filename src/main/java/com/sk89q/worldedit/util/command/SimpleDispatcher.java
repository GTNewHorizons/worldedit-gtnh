...
public class SimpleDispatcher implements Dispatcher {
    ...
    public SimpleDispatcher() {
        description.getParameters()
            .add(new SimpleParameter("subcommand"));
    ...
    public Object call(String arguments, CommandLocals locals, String[] parentCommands) throws CommandException {
        ...
        if (aliases.isEmpty()) {
            ...
            if (mapping != null) {
                try {
                    return mapping.getCallable()
                        .call(subArguments, locals, subParents);
    ...
    public List<String> getSuggestions(String arguments, CommandLocals locals) throws CommandException {
        ...
        if (split.length <= 1) {
            ...
            for (CommandMapping mapping : getCommands()) {
                if (mapping.getCallable()
                    .testPermission(locals)) {
            ...
            String subCommand = split[0];
            CommandMapping mapping = get(subCommand);
            String passedArguments = Joiner.on(" ")
                .join(Arrays.copyOfRange(split, 1, split.length));

            if (mapping != null) {
                return mapping.getCallable()
                    .getSuggestions(passedArguments, locals);
            } else {
