...
class ParametricCallable implements CommandCallable {
    ...
    ParametricCallable(ParametricBuilder builder, Object object, Method method, Command definition)
        ...
        String[] names = builder.getParanamer()
            .lookupParameterNames(method, false);
            ...
            if (parameter.isOptional() && parameter.getFlag() == null) {
                ...
                if (numOptional > 0 && parameter.isNonFlagConsumer()) {
                    if (parameter.getConsumedCount() < 0) {
                        throw new ParametricException(
                            "Found an parameter using the binding " + parameter.getBinding()
                                .getClass()
                                .getCanonicalName()
        ...
        description.setDescription(
            !definition.desc()
                .isEmpty() ? definition.desc() : null);
        description.setHelp(
            !definition.help()
                .isEmpty() ? definition.help() : null);
        description.overrideUsage(
            !definition.usage()
                .isEmpty() ? definition.usage() : null);
        ...
        try {
            ...
            for (int i = 0; i < parameters.length; i++) {
                ...
                if (mayConsumeArguments(i, arguments)) {
                    ...
                    try {
                        args[i] = parameter.getBinding()
                            .bind(parameter, usedArguments, false);
    ...
    public List<String> getSuggestions(String arguments, CommandLocals locals) throws CommandException {
        return builder.getDefaultCompleter()
    ...
    public boolean testPermission(CommandLocals locals) {
        if (commandPermissions != null) {
            for (String perm : commandPermissions.value()) {
                if (builder.getAuthorizer()
                    .testPermission(locals, perm)) {
                    return true;
                }
            }
        ...
        if (defaultValue != null) {
            try {
                return parameter.getBinding()
                    .bind(parameter, new StringArgumentStack(context, defaultValue, false), false);
            } catch (MissingParameterException e) {
                throw new ParametricException(
                    "The default value of the parameter using the binding " + parameter.getBinding()
    ...
    private static String generateName(Type type, Annotation classifier, int index) {
        if (classifier != null) {
            return classifier.annotationType()
                .getSimpleName()
                .toLowerCase();
        } else {
