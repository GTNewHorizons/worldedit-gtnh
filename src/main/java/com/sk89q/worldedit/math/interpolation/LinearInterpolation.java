...
public class LinearInterpolation implements Interpolation {
    ...
    public Vector getPosition(double position) {
        ...
        final Vector position1 = nodes.get(index1)
            .getPosition();
        final Vector position2 = nodes.get(index1 + 1)
            .getPosition();

        return position1.multiply(1.0 - remainder)
            .add(position2.multiply(remainder));
    }
    ...
    private double arcLengthRecursive(int index, double remainderA, double remainderB) {
        final Vector position1 = nodes.get(index)
            .getPosition();
        final Vector position2 = nodes.get(index + 1)
            .getPosition();
