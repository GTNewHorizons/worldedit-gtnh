    ...
    public void testTransform() throws Exception {
        ...
        for (BlockType type : BlockType.values()) {
            ...
            for (int i = 1; i < 4; i++) {
                ...
                assertThat(
                    type + "#"
                        ...
                        + " rotated "
                        + (90 * i)
                        + " degrees did not match BlockData.rotate90()'s expected result",
                    rotated,
                    equalTo(reference));
                orig = rotated;
            }
            ...
            for (int i = 0; i < 4; i++) {
                ...
                BaseBlock reference = new BaseBlock(
                    orig.getType(),
                    BlockData.rotate90Reverse(orig.getType(), orig.getData()));
                assertThat(
                    type + "#"
                        + type.getID()
                        + " rotated "
                        + (-90 * i)
                        + " degrees did not match BlockData.rotate90Reverse()'s expected result",
                    rotated,
                    equalTo(reference));
                orig = rotated;
            }
